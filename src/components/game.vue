...
const isCanMove = computed(() => {
  const moveIdx = store.moveIndex;
  if (localStorage.getItem("username") === users.value[0])
    return moveIdx % 2 == 0;
...
<template>
  <div
    v-if="log"
    class="fixed inset-0 flex items-center justify-center bg-red-600 text-white text-2xl"
  >
    {{ log }}
  </div>
  <div
    v-if="users.length < 2"
    class="fixed inset-0 flex flex-col gap-4 items-center justify-center bg-black text-white text-2xl"
  >
  ...
  <div
    class="w-full max-w-7xl mx-auto h-screen flex justify-between py-2 items-center flex-col"
  >
    <div class="w-full flex flex-row items-center justify-around">
      <p class="text-white" v-for="(user, index) in users" :key="index">
        {{ user }}
      </p>
    </div>
    <p v-if="users.length > 0" class="text-white">
      {{
        store.moveIndex % 2 == 0
          ? `User ${users[0]} move`
          : `User ${users[1]} move`
    ...
    <div
      class="grid grid-cols-3 gap-2 w-full s:w-1/2 max-w-7xl mx-auto h-1/2 justify-center"
    >
      <div
        v-for="(item, index) in matrix"
        :key="index"
        @click="item.length < 1 && isCanMove ? userMove(index) : ''"
